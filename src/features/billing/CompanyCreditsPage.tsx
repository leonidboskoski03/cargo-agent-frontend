  ...
  return (
    <div className="space-y-6">
      ...
      {checkoutMutation.error ? (
        <ErrorState description="Company credit checkout could not be started. Check Stripe sandbox price setup and webhook readiness." error={checkoutMutation.error} title="Unable to start checkout" />
      ) : null}
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {packs.map((pack) => (
              <div className="rounded-lg border border-border bg-surface-pearl p-4" key={pack.id}>
                ...
                <Button className="mt-4 w-full" disabled={!canManage || checkoutMutation.isPending || !pack.isActive} onClick={() => checkoutMutation.mutate({ creditPackCode: pack.code, idempotencyKey: idempotencyKey(pack.code) })} type="button">
